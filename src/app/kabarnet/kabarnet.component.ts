...
export class KabarnetComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.calendarOptions = {
      ...
      allDaySlot: false,
      eventColor: "#064dae",
      events: environment.APIurl + '/api/board/appointments',
      eventMaxStack: 2,
  ...
  handleUpdateData(intel: any) {
    ...
    if (username == tokenName.username) {
      ...
      this.http
        .put(environment.APIurl + `/board/update/${changeId}`, uploadData)
        .subscribe(
          (response: any) => (
  ...
  test() {
    ...
    // uploadData.append("color", this.colorScheme);
    // console.log(this.colorScheme, "hey")
    this.http.post(environment.APIurl + '/api/board/appointments', uploadData).subscribe(
      (response: any) => (
        // console.log(response),
  ...
  // the delete popup🙄
  onDeletePopup(){
    this.http.delete(environment.APIurl +  `/board/update/${this.idddddd}`).subscribe((info:any) => {
      this.snack.open("You have deleted your schedule", "cancel")
      setTimeout(() => {
        location.reload();
      }, 1000)
  ...
  postData() {
    ...
    this.http.post(environment.APIurl + '/api/board/appointments' ,uploadData).subscribe((response:any) => {
      // console.log(response)
